<template>
  <div>
    ...
    <!-- 弹窗 -->
    <el-dialog :title="isAdd?'新增'+tip:'编辑'+tip" :visible.sync="isShow" @open="openFn" @close="closeFn">
      <!-- 表单 -->
      <el-form :model="form" :rules="rules" label-width="200px" ref="form">
        ...
        <el-form-item label="类型">
          <el-radio v-model="form.type" :label="1" :disabled="form.type == 2">目录</el-radio>
          <el-radio v-model="form.type" :label="2" :disabled="form.type == 1">菜单</el-radio>
        </el-form-item>
  ...
  methods: {
    ...
    async handleDelete(row) {
      try {
        ...
        // 进行删除操作，点击了删除确定按钮
        let res = await this.$http.post("/api/menudelete",{id:row.id});
        console.log(res);
        if(res.code==200){
          this.tableData =[...res.list]
        }
